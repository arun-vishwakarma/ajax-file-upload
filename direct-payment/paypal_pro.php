...
function PPHttpPost($methodName_, $nvpStr_) {
	...
	$API_UserName = urlencode('arun_biz_api1.paypal.com'); // set your spi username
	$API_Password = urlencode('BCH2KBYHBSA2HRH7'); // set your spi password
	$API_Signature = urlencode('An5ns1Kso7MWUdW4ErQKJJJ4qi4-AoQ.Z7tQ8Q16-cs1BuKJgr0KnK11'); // set your spi Signature
...
$SHIPTONAME = 'CHRIS NISWANDEE';
$SHIPTOSTREET = '795 E DRAGRAM';
$SHIPTOCITY = 'TUCSON';
$SHIPTOSTATE = 'AZ';
$SHIPTOZIP = '85705';
$SHIPTOCOUNTRY = 'US';
...
$nvpStr =	"&PAYMENTACTION=$paymentType&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber".
			...
			"&STREET=$address1&CITY=$city&STATE=$state&ZIP=$zip&COUNTRYCODE=$country&CURRENCYCODE=$currencyID&SHIPTONAME=$SHIPTONAME&SHIPTOSTREET=$SHIPTOSTREET&SHIPTOCITY=$SHIPTOCITY&SHIPTOSTATE=$SHIPTOSTATE&SHIPTOZIP=$SHIPTOZIP&SHIPTOCOUNTRY=$SHIPTOCOUNTRY&DESC=$DESC";
 
// Execute the API operation; see the PPHttpPost function above.
$httpParsedResponseAr = PPHttpPost('DoDirectPayment', $nvpStr);
 
if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
	exit('Direct Payment Completed Successfully: <pre>'.print_r($httpParsedResponseAr, true));
} else  {
	exit('DoDirectPayment failed: <pre>' . print_r($httpParsedResponseAr, true));
}
